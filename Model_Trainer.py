...
def train_pytorch_model(model, X_test_tensor, train_loader_local, task_type):
    ...
    criterion = (
        nn.BCEWithLogitsLoss() if task_type == "classification" else nn.MSELoss()
    )
    ...
    y_pred_probs_or_values = pytorch_predict_wrapper(
        model, X_test_tensor.numpy(), task_type
    )
...
for vehicle_type, config in DATA_CONFIG.items():
    ...
    if vehicle_type == "engine":
        ...
        data_filtered = data[
            numerical_features + categorical_features + [config["target"]]
        ]
        data = data_filtered
    ...
    joblib.dump(
        preprocessor, os.path.join("models", f"{vehicle_type}_preprocessor.pkl")
    )
    ...
    if choice in ["1", "2", "3"]:
        ...
        model, y_pred, training_time = train_pytorch_model(
            model, X_test_tensor, train_loader, task_type
        )

        # Save PyTorch model state dict
        torch.save(
            model.state_dict(), os.path.join("models", f"{vehicle_type}_{base_name}")
        )
    ...
    # --- Machine Learning Model Training (Choice 4-7) ---
        ...
        if choice == "4":
            ...
            model = (
                LGBMClassifier(**model_kwargs_silent)
                if task_type == "classification"
                else LGBMRegressor(**model_kwargs_silent)
            )
            ...
            meta_model = XGBM(
                eval_metric="logloss", use_label_encoder=False, **model_kwargs
            )
            model = StackM(
                estimators=base_models, final_estimator=meta_model, n_jobs=N_JOBS
            )
        ...
        print(
            f"✅ {model.__class__.__name__} model saved as {vehicle_type}_{base_name}"
        )
    ...
    if y_pred is None:
        print(
            "❌ Evaluation skipped: Model training failed or y_pred was not generated."
        )
        continue
