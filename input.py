...
# Attempt to import voice-related dependencies
try:
    ...
    print("⚠️ Voice recognition dependencies not found.")
    STT_AVAILABLE = False
...
def parse_number(text: str) -> Union[float, None]:
    """Converts spoken or typed numbers to float."""
    ...
    for t in text.split():
        try:
            ...
            if re.fullmatch(r'\d*\.\d+|\d+', t) or t == '.':
                tokens.append(t)
            elif t in ('oh', 'o'):
                tokens.append('0')
...
def get_input_data(speak_func, listen_func, parse_func, input_features: List[str],
                   vehicle_choice_key: str = "", input_mode: str = "text") -> Dict[str, Union[str, float]]:
    """
        ...
        if numeric_features:
            while True:
                ...
                if len(values) != len(numeric_features):
                    print(f"❌ Expected {len(numeric_features)} values, got {len(values)}. Try again.")
                    continue
                try:
            ...
            raw_input_dict.update(dict(zip(numeric_features, values)))
        ...
        for feat in input_features:
            while True:
                ...
                if feat == "product ID" and vehicle_choice_key == "engine":
                    speak_func("Please say the product quality variant: Low, Medium, or High.")
                    ...
                    if val is not None:
                        speak_func(f"I recorded {val} {unit}. Say 'yes' to confirm or 'no' to repeat.")
                        confirm = listen_func()
                        ...
                        speak_func(f"Could not recognize a valid number. Let's repeat {feat}.")
        print(f"\n✅ Final input list: {list(raw_input_dict.values())}")
        return raw_input_dict
