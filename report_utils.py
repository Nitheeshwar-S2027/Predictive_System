...
# Ensure fpdf2 is installed (pip install fpdf2) for this code to work.
...
def generate_suggestions(status: str, top_feature: str) -> List[str]:
    """Generates maintenance suggestions based on the predicted status."""
    base_suggestions = [
    ...
    return ['Review historical data trends to detect subtle degradation.'] + base_suggestions


class MaintenancePDF(FPDF):
    """Custom FPDF class for the professional report structure, using Helvetica for Unicode support."""
    ...
    def header(self):
        ...
        self.cell(0, 10, 'Predictive Maintenance Diagnostic Report', new_y=YPos.NEXT, align='C', fill=False)
        self.set_font('Helvetica', '', 10)
        self.cell(0, 5, datetime.now().strftime("Report Generated: %Y-%m-%d %H:%M:%S"), new_y=YPos.NEXT, align='C')
        self.ln(5)
    ...
    def chapter_title(self, title):
        ...
        self.line(self.get_x(), self.get_y() - 1, self.w - self.r_margin, self.get_y() - 1)
        self.ln(5)
        self.set_text_color(0, 0, 0)

    def status_box(self, status_message, status_color_name, rul_cycles, vehicle_type):
        ...
        if self.get_string_width(status_text) > max_width:
            self.set_font('Helvetica', 'B', 14) # Switched to Helvetica
        self.cell(100, 10, f'CURRENT STATUS: {status_text}', align='L', fill=False)

        # if vehicle_type == 'ev':
        #     self.set_xy(110, self.get_y()) 
        #     self.set_font('Helvetica', 'B', 14) # Switched to Helvetica
        #     self.cell(90, 10, f'RUL: {int(rul_cycles):,} cycles', align='R', fill=False)

        self.ln(20) 
        self.set_text_color(0, 0, 0) 
        self.set_font('Helvetica', '', 10) # Switched to Helvetica
        self.ln(5)
    ...
    def data_table(self, raw_input: Dict[str, Union[str, float]]):
        ...
        self.cell(col_widths[0], 7, 'Parameter', 1, new_x=XPos.RIGHT, new_y=YPos.TOP, align='L', fill=True)
        self.cell(col_widths[1], 7, 'Value', 1, new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='L', fill=True)
        self.set_font('Helvetica', '', 10) # Switched to Helvetica
        ...
        for idx, (name, value) in enumerate(raw_input.items()):
            ...
            if idx % 2 == 1:
                self.set_fill_color(*COLOR_GRAY)
            self.cell(col_widths[0], 6, safe_name, 'LR', new_x=XPos.RIGHT, new_y=YPos.TOP, align='L', fill=True)
            self.cell(col_widths[1], 6, safe_value, 'LR', new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='R', fill=True)

        self.cell(sum(col_widths), 0, '', 'T', new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='C')
        self.ln(5)
    ...
    def suggestions_list(self, suggestions: List[str]):
        ...
        for item in suggestions:
            self.set_text_color(*COLOR_BLUE)
            # This is the line that required the Unicode font (Helvetica)
            self.cell(5, 7, '-', 0, new_x=XPos.RIGHT, new_y=YPos.TOP, align='L') 
            self.set_text_color(0, 0, 0)
            if 'URGENT' in item or 'immediate diagnostic' in item:
                self.set_font('Helvetica', 'B', 10) # Switched to Helvetica
                self.set_text_color(*COLOR_RED)
            self.multi_cell(self.w - 2*self.l_margin, 7, item, 0, 'L')
            self.set_font('Helvetica', '', 10) # Switched to Helvetica
...
def generate_pdf_report(
    ...
    # Load the built-in Unicode-compatible font (Helvetica)
    # This is CRITICAL for the bullet point (•) and other special characters.
    ...
    pdf.set_font('Helvetica', '', 10) # Switched to Helvetica
    
    # Text formatting and cleanup
    explanation_summary = explanation_summary.replace(top_feature, f'***{top_feature.replace("_", " ")}***')
    explanation_summary = explanation_summary.replace('**', '')
    
    pdf.set_x(pdf.l_margin) 
    pdf.multi_cell(pdf.w - 2*pdf.l_margin, 5, explanation_summary)
    
    pdf.set_font('Helvetica', 'B', 10) # Switched to Helvetica
    pdf.ln(3)
    pdf.cell(0, 5, f'Most Influential Feature: {top_feature.replace("_", " ")}', new_y=YPos.NEXT, align='L')
    pdf.ln(5)
...
def send_real_email(recipient: str, subject: str, body: str, attachment_path: str):
    """Sends the PDF report via email using SMTP."""
    if SENDER_EMAIL == "your_sender_email@gmail.com" or SENDER_PASSWORD == "your_app_password_here":
        speak("❌ WARNING: SENDER_EMAIL or SENDER_PASSWORD not configured. Skipping real email send.")
        speak("Please set SENDER_EMAIL and SENDER_PASSWORD variables (use an App Password for Gmail).")
        return False
    ...
    try:
        ...
        speak("❌ ERROR: Email failed. Check your SENDER_EMAIL/SENDER_PASSWORD (use App Password for Gmail).")
...
def handle_report_and_email(
    ...
    """
    Coordinates PDF generation and email dispatch.
    This function is intended to be called from predictor.py.
    """
    speak("Starting professional predictive maintenance report generation (PDF format).")
    
    REPORTS_DIR = "reports"
    os.makedirs(REPORTS_DIR, exist_ok=True)
    
    output_filename_base = f"report_{vehicle_choice_key}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    ...
    try:
        ...
        speak("Ensure 'fpdf2' is installed (pip install fpdf2) and all parameters are correct.")
        return

    recipient_email = input("Please manually enter the recipient email for the report: ")
    ...
    email_body = f"Dear Team,\n\nA Predictive Maintenance Report has been generated for a {vehicle_choice_key.upper()} unit.\n\n"
    email_body += f"STATUS: {status_message.replace('**', '')}\n"
    email_body += f"Influential Factor: {top_feature.replace('_', ' ')}\n\n"
    email_body += f"Please find the full diagnostic report attached as a PDF file."
    
    send_real_email(recipient_email, subject, email_body, pdf_path)

    speak(f"✅ Process complete. Check your reports folder for '{output_filename_base}'.")